...
class Game:
    ...
    def __init__(self, world_size, creature, no_rewards_ends=True, rewards=default_rewards, penalties=default_move_penalties):
        """
        Initialize a game
        ...
        Args:
            ...
            rewards:  The rewards for the game
            penalties: The move penalties for the game
        ...
        self.move_penalties = penalties
        self.no_rewards_ends = no_rewards_ends
        self.build_world()
        self.make_board()
        self.draw_board()
    ...
    def make_board(self):
        ...
        """
        url_tiles = 'http://www.cse.msu.edu/~ruppmatt/itm891/tiles.pickle'
        with urllib.request.urlopen(url_tiles) as response:
            pickle_tiles = response.read()
        tiles = pickle.loads(pickle_tiles)
        self.assets = tiles
    ...
    def no_more_positive_rewards(self):
        list_rewards = list(itertools.chain.from_iterable(self.world_rewards))
        to_return =  len(list(filter(lambda x: x != None and x.points > 0, list_rewards)))
        return to_return == 0
    ...
    def play(self, play_func, render=False, data=None):
        ...
        if self.no_rewards_ends:
            return self.no_more_positive_rewards()
